...
sub analyze_combo
	...
	@combolist=("I__","_E_", "IE_",  "__L",  "I_L", "_EL","IEL");
	%H=();
	%B=();
	%NIN=();
	open (F, $file);
	...
	foreach $ref ("I__", "_E_", "__L")
	{
		...
		foreach $bst ("000", "080", "100")
		{
			...
			foreach $combo2 (@combolist)
			{
			    ...
			    $finalauc{$ref.$ppmode.$comboN}{sd}   =$auc{$comboN}{sd};
...
sub print_table
{
	...
	foreach $c2 (@combolist)
	{
	    ...
	    printf ("%.3f +/-%.2f\t", $finalauc{$measure}{$content},$finalauc{$measure}{sd});
    ...
    my %done;
    printf ("combo\t");
    foreach my $c1(@combolist){printf ("$c1\t");}
    printf ("\n");
    foreach my $c1(@combolist)
    {
	printf ("$c1\t");
	foreach my $c2 (@combolist)
	{
	    my @list1;
	    my @list2;
	    
	    for (my $a=0; $a<$#combolist; $a++)
	    {
	    ...
	    printf ("%.5f\t", $wilcox_test->probability());
	}
	printf ("\n");
    }
...
sub h2auc
{
	...
	foreach my $combo (@combolist)
	{
		...
		$auc{$comboN}{sd }  +=$H{$f}{$ref}{$ppmode}{$comboN}{auc}*$H{$f}{$ref}{$ppmode}{$comboN}{auc};
	...
	if ($auc{$k}{n}>0)
	{
	    ...
	    $auc{$k}{auc}/=$auc{$k}{n};
	    $auc{$k}{isone}/=$auc{$k}{n};
	    $auc{$k}{sd}=sqrt( $auc{$k}{sd}/$auc{$k}{n} -  $auc{$k}{auc}* $auc{$k}{auc})
	}
    }
    return %auc;
}
