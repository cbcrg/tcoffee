...
tfile=\",1,0));\n  }\nelsif ( $mode eq \"psiprofil\
e_template\")\n  {\n    &psiblast2profile_template\
 ($mode,&my_get_opt ( $cl, \"-infile=\",1,1, \"-da\
tabase=\",1,0, \"-method=\",1,0, \"-outfile=\",1,0\
));\n  }\nelsif ( $mode eq \"RNA_template\")\n  {\\
n    &seq2RNA_template ($mode,&my_get_opt ( $cl, \\
"-infile=\",1,1, \"-outfile=\",1,0));\n  }\nelsif \
( $mode eq \"tm_template\")\n  {\n    &seq2tm_temp\
late ($mode, \"\", &my_get_opt ( $cl, \"-infile=\"\
,1,1,\"-arch=\",1,1,\"-psv=\",1,1, \"-outfile=\",1\
,0,));\n  }\nelsif ( $mode eq \"psitm_template\")\\
n  {\n    &seq2tm_template ($mode,&my_get_opt ( $c\
l, \"-database=\",1,0, \"-infile=\",1,1, \"-arch=\\
",1,1,\"-psv=\",1,1, \"-outfile=\",1,0,));\n  }\ne\
lsif ( $mode eq \"ssp_template\")\n  {\n    &seq2s\
sp_template ($mode,&my_get_opt ( $cl, \"-infile=\"\
,1,1,\"-seq=\",1,1,\"-obs=\",1,1, \"-outfile=\",1,\
0));\n  }\nelsif ( $mode eq \"psissp_template\")\n\
  {\n    &seq2ssp_template ($mode,&my_get_opt ( $c\
l, \"-infile=\",1,1,\"-seq=\",1,1,\"-obs=\",1,1, \\
"-outfile=\",1,0));\n  }\n\nelsif ( $mode eq \"rna\
_pair\")\n{\n    &seq2rna_pair($mode,&my_get_opt (\
 $cl, \"-pdbfile1=\",1,1, \"-pdbfile2=\",1,1, \"-m\
ethod=\",1,2,\"-param=\",0,0, \"-outfile=\",1,0, )\
);\n}\nelsif ( $mode eq \"calc_rna_template\")\n{\\
n    &calc_rna_template($mode,&my_get_opt ( $cl, \\
"-infile=\",1,1,\"-pdbfile=\",1,1, \"-outfile=\",1\
,0));\n}\nelse\n  {\n    myexit(flush_error( \"$mo\
de is an unknown mode of tc_generic_method.pl\"));\
\n  }\nmyexit ($EXIT_SUCCESS);\n\n\nsub seq2ssp_te\
...
\", $lib_name,$gor_seq, $gor_obs);\n	}\n    \n    \
  if ( !-e $lib_name)\n	{\n	  myexit(flush_error(\\
"GORIV failed to compute the secondary structure o\
f $s{$seq}{name}\"));\n	  myexit ($EXIT_FAILURE);\\
n	}\n      else\n	{\n	  print stdout \"\\tProcess:\
 >$s{$seq}{name} _E_ $lib_name \\n\";\n	  print R \
\">$s{$seq}{name} _E_ $lib_name\\n\";\n	}\n      u\
nshift (@profiles, $lib_name);\n    }\n  close (R)\
...
fasta.pl -in=seqfile -out=$lib_name -arch=$arch -p\
sv=$psv\");\n	}\n      elsif ( $mode eq \"psitm_te\
mplate\")\n	{\n	  &seq2msa_tm_prediction ($s{$seq}\
{name},$s{$seq}{seq}, $db, \"seqfile\", $lib_name,\
$arch, $psv);\n	}\n      if ( !-e $lib_name)\n	{\n\
	  myexit(flush_error(\"RNAplfold failed to comput\
e the secondary structure of $s{$seq}{name}\"));\n\
	  myexit ($EXIT_FAILURE);\n	}\n      else\n	{\n	 \
 print stdout \"\\tProcess: >$s{$seq}{name} _T_ $l\
ib_name\\n\";\n	  print R \">$s{$seq}{name} _T_ $l\
ib_name\\n\";\n	}\n      unshift (@profiles, $lib_\
name);\n    }\n  close (R);\n  &set_temporary_dir \
...
\n      print (F \">$s{$seq}{name}\\n$s{$seq}{seq}\
\\n\");\n      close (F);\n      $lib_name=\"$s{$s\
eq}{name}.rfold\";\n      $lib_name=&clean_file_na\
me ($lib_name);\n      &safe_system (\"t_coffee -o\
ther_pg RNAplfold2tclib.pl -in=seqfile -out=$lib_n\
ame\");\n      \n      if ( !-e $lib_name)\n	{\n	 \
myexit(flush_error(\"RNAplfold failed to compute t\
he secondary structure of $s{$seq}{name}\"));\n	  \
myexit ($EXIT_FAILURE);\n	}\n      else\n	{\n	  pr\
int stdout \"\\tProcess: >$s{$seq}{name} _F_ $lib_\
name\\n\";\n	  print R \">$s{$seq}{name} _F_ $lib_\
name\\n\";\n	}\n      unshift (@profiles, $lib_nam\
e);\n    }\n  close (R);\n  &set_temporary_dir (\"\
...
\";\n      }\n    close ($P);\n\n    if ( $trim)\n\
      {\n	&safe_system (\"t_coffee -other_pg seq_r\
eformat -in $tmp -action +trim _aln_%%$trim\\_K1 -\
output fasta_aln -out $outfile\");\n      }\n    e\
lse\n      {\n	&safe_system (\"mv $tmp $outfile\")\
;\n      }\n    return;\n  }\nsub blast_xml2hit_li\
st\n  {\n    my $string=(@_[0]);\n    return &xml2\
tag_list ($string, \"hit\");\n  }\nsub xmltag2valu\
e\n  {\n    my ($string_in, $tag)=@_;\n    my %TAG\
...
$gor_obs)=(@_);\n    my ($l);\n    \n    `gorIV -p\
rd $infile -seq $gor_seq -obs $gor_obs > gor_tmp`;\
\n    open (GR, \">$outfile\");\n    open (OG, \"g\
or_tmp\");\n\n    while (<OG>)\n      {\n	\n	$l=$_\
;\n	if ($l=~/\\>/){print GR \"$l\";}\n	elsif ( $l=\
...
name,\"blastp\", $db, $infile, \"outfile\");\n    \
\n    if (&cache_file(\"GET\",$infile,$name,$metho\
d,$db,$outfile,$SERVER))\n      {\n	print \"\\tPSI\
TM: USE Cache\\n\";\n	return $outfile;\n      }\n \
   else\n      {\n	$CACHE_STATUS=\"COMPUTE CACHE\"\
;\n	%p=blast_xml2profile($name,$seq,$maxid, $minid\
,$mincov,$blast_output);\n	\n	\n	open (F, \">tm_in\
put\");\n	for (my $a=0; $a<$p{n}; $a++)\n	  {\n	  \
  my $s;\n	    \n	    $s=$p{$a}{seq};\n	    $s=uc(\
$s);\n	    print F \">$p{$a}{name}\\n$s\\n\";\n	  \
  #print stdout \">$p{$a}{name}\\n$s\\n\";\n	  }\n\
	close (F);\n	print \"\\tPSITM: kept  $p{n} Homolo\
gues for Sequence $p{0}{name}\\n\";\n	&safe_system\
 (\"t_coffee -other_pg fasta_seq2hmmtop_fasta.pl -\
in=tm_input -out=$outfile -output=cons -cov=70 -tr\
im=95 -arch=$arch -psv=$psv\");\n	unlink (\"tm_inp\
ut\");\n	&cache_file(\"SET\",$infile,$name,$method\
,$db,$outfile,$SERVER);\n	return;\n      }\n  }\n\\
n\nsub seq2msa_gor_prediction \n  {\n    my ($name\
, $seq,$infile, $outfile, $gor_seq, $gor_obs)=(@_)\
;\n    my (%p,%gseq,%R, $blast_output, %s, $l);\n \
   my $R2=new FileHandle;\n    my $db=\"uniprot\";\
\n    my $method=\"psigor\";\n    my $SERVER=\"EBI\
\";\n    \n    $blast_output=&run_blast ($name,\"b\
lastp\", \"uniprot\", $infile, \"outfile\");\n    \
\n    if (&cache_file(\"GET\",$infile,$name,$metho\
d,$db,$outfile,$SERVER))\n      {\n	print \"\\tPSI\
GOR: USE Cache\\n\";\n	return $outfile;\n      }\n\
    else\n      {\n	$CACHE_STATUS=\"COMPUTE CACHE\\
";\n	%p=blast_xml2profile($name,$seq,$maxid, $mini\
d,$mincov,$blast_output);\n	\n	\n	open (F, \">gor_\
input\");\n	for (my $a=0; $a<$p{n}; $a++)\n	  {\n	\
    my $s;\n	    \n	    $s=$p{$a}{seq};\n	    $s=u\
c($s);\n	    print F \">$p{$a}{name}\\n$s\\n\";\n	\
    #print stdout \">$p{$a}{name}\\n$s\\n\";\n	  }\
...
wu/)\n	      {\n		$cl_method=~s/wu//;\n		if ( $cl_\
method eq \"psiblast\")\n		  {\n		    add_warning(\
$$,$$,\"PSI BLAST cannot be used with the wuBLAST \
Client. Use server=EBI Or server=LOCAL. blastp wil\
l be used instead\");\n		    $cl_method=\"blastp\"\
;\n		  }\n		\n		$command=\"t_coffee -other_pg wubl\
ast.pl --email $EMAIL $infile -D $db -p $cl_method\
 --outfile $outfile -o xml>/dev/null 2>/dev/null\"\
;\n		&safe_system ( $command);\n		if (-e \"$outfil\
e.xml\") {`mv $outfile.xml $outfile`;}\n	      }\n\
	    else\n	      {\n		if ($cl_method eq \"psiblas\
t\"){$cl_method =\"blastp -j5\";}\n		\n		$command=\
\"t_coffee -other_pg blastpgp.pl --email $EMAIL $i\
nfile -d $db --outfile $outfile -p $cl_method --mo\
de PSI-Blast>/dev/null 2>/dev/null\";\n		&safe_sys\
tem ( $command);\n		\n		if (-e \"$outfile.xml\") {\
`mv $outfile.xml $outfile`;}\n	      }\n	  }\n	els\
if ($SERVER eq \"EBI_REST\" || $SERVER eq \"EBI\")\
\n	  {\n	   \n	    $cl_method=$method;\n	    &chec\
k_configuration(\"EMAIL\",\"XML::Simple\", \"INTER\
NET\");\n	    if ($db eq \"uniprot\"){$db1=\"unipr\
otkb\";}\n	    else {$db1=$db;}\n	    \n\n	    if \
($cl_method =~/wu/)\n	      {\n		$cl_method=~s/wu/\
...
at xml --stype protein $infile>/dev/null 2>/dev/nu\
ll\";\n		&safe_system ( $command,5);\n		if (-e \"$\
outfile.xml\") {`mv $outfile.xml $outfile`;}\n		el\
sif (-e \"$outfile.xml.xml\"){`mv $outfile.xml.xml\
 $outfile`;}\n	      }\n	    else\n	      {\n		if \
( $cl_method =~/psiblast/){$cl_method =\"blastp -j\
5\";}\n		$command=\"t_coffee -other_pg ncbiblast_l\
wp.pl --email $EMAIL -D $db1 -p $cl_method --outfi\
le $outfile --outformat xml --stype protein $infil\
e>/dev/null 2>/dev/null\";\n		#$command=\"t_coffee\
 -other_pg ncbiblast_lwp.pl --email $EMAIL -D $db1\
 -p $cl_method --outfile $outfile --outformat xml \
--stype protein $infile>/dev/null\";\n		&safe_syst\
em ( $command,5);\n		if (-e \"$outfile.xml\") {`mv\
 $outfile.xml $outfile`;}\n		elsif (-e \"$outfile.\
xml.xml\"){`mv $outfile.xml.xml $outfile`;} \n		\n\
	      }\n	    \n	 }\n	elsif ($SERVER eq \"NCBI\")\
\n	  {\n	    &check_configuration (\"blastcl3\",\"\
INTERNET\");\n	    if ($db eq \"uniprot\"){$cl_db=\
\"nr\";}\n	    else {$cl_db=$db;}\n	    \n	    if \
( $method eq \"psiblast\")\n	      {\n		add_warnin\
g($$,$$,\"PSI BLAST cannot be used with the NCBI B\
LAST Client. Use server=EBI Or server=LOCAL. blast\
p will be used instead\");\n		$cl_method=\"blastp\\
";\n	      }\n	    else\n	      {\n		$cl_method=$m\
ethod;\n	      }\n	    $command=\"blastcl3 -p $cl_\
method -d $cl_db -i $infile -o $outfile -m 7\";\n	\
    &safe_system ($command);\n	  }\n	elsif ($SERVE\
R =~/CLIENT_(.*)/)\n	  {\n	    my $client=$1;\n	  \
  $command=\"$client -p $method -d $db -i $infile \
-o $outfile -m 7\";\n	    &safe_system ($command);\
\n	  }\n	elsif ( $SERVER eq \"LOCAL_blastall\")\n	\
  {\n	    &check_configuration (\"blastall\");\n	 \
   if ($method eq \"blastp\")\n	      {\n		$comman\
d=\"blastall -d $db -i $infile -o $outfile -m7 -p \
blastp\";\n	      }\n	    &safe_system ($command);\
...
and=\"blastpgp -d $cl_db -i $infile -o $outfile -m\
7 -j1\";\n	      }\n	    elsif ($method eq \"psibl\
ast\")\n	      {\n		&check_configuration(\"blastpg\
p\");\n		$command=\"blastpgp -d $cl_db -i $infile \
-o $outfile -m7 -j5\";\n	      }\n	    elsif ($met\
hod eq \"blastn\")\n	      {\n		&check_configurati\
on(\"blastall\");\n		$command=\"blastall -p blastn\
...
blast_xml($outfile))\n	  {\n	    \n	    if ( -e $o\
utfile)\n	      {\n		add_warning ($$,$$,\"Corrupte\
d Blast Output (Run $run)\");\n		unlink($outfile);\
\n	      }\n	    \n	    if ( $run==$BLAST_MAX_NRUN\
S)\n	      {\n	\n		myexit(add_error (EXIT_FAILURE,\
$$,$$,getppid(), \"BLAST_FAILURE::UnknownReason\",\
 \"$command\"));\n	      }\n	    else\n	      {\n	\
\n		add_warning ($$,$$,\"Blast for $name failed (R\
un: $run)\");\n		\n		return run_blast ($name, $met\
hod, $db,$infile, $outfile, $run+1);\n	      }\n	 \
 }\n	\n	&cache_file(\"SET\",$infile,$name,$method,\
$db,$outfile,$SERVER);\n	return $outfile;\n      }\
\n  }\n\nsub cache_file\n  {\n    my ($cache_mode,\
$infile,$name,$method,$db, $outfile,$server)=(@_);\
\n    my $cache_file;\n    #Protect names so that \
they can be turned into legal filenames\n    $name\
=&clean_file_name ($name);\n\n    if ($db=~/\\//)\\
n      {\n	$db=~/([^\\/]+)$/;\n	$db=$1;\n      }\n\
    $cache_file_sh=\"$name.$method.$db.$server.tmp\
\";\n    $cache_file=\"$CACHE/$name.$method.$db.$s\
erver.tmp\";\n    \n    if ($infile ne \"\")\n    \
  {\n	$cache_file_infile_sh=\"$name.$method.$db.$s\
erver.infile.tmp\";\n	$cache_file_infile=\"$CACHE/\
$name.$method.$db.$server.infile.tmp\";\n      }\n\
    \n    if ($cache_mode eq \"GET\")\n      {\n	i\
f ($CACHE eq \"\" || $CACHE eq \"no\" || $CACHE eq\
 \"ignore\"  || $CACHE eq \"local\" || $CACHE eq \\
"update\"){return 0;}\n	elsif ( !-d $CACHE)\n	  {\\
n	    print STDERR \"ERROR: Cache Dir: $CACHE Does\
 not Exist\";\n	    return 0;\n	  }\n	else\n	  {\n\
	    if ( -e $cache_file && &fasta_file1_eq_fasta_\
file2($infile,$cache_file_infile)==1)\n	      {\n	\
...
if ($cache_file_infile ne \"\"){ `cp $infile $cach\
e_file_infile`;}\n\n	    #functions for updating t\
he cache\n	    #`t_coffee -other_pg clean_cache.pl\
 -file $cache_file_sh -dir $CACHE`;\n	    #`t_coff\
ee -other_pg clean_cache.pl -file $cache_file_infi\
le_sh -dir $CACHE`;\n	    return 1;\n	  }\n      }\
\n    $CACHE_STATUS=\"COMPUTE CACHE\";\n    return\
 0;\n  }\nsub file1_eq_file2\n  {\n    my ($f1, $f\
2)=@_;\n    if ( $f1 eq \"\"){return 1;}\n    elsi\
...
($mode, $infile, $pdbfile, $outfile)=@_;\n	my %s, \
%h ;\n	my $result;\n	my (@profiles);\n	&set_tempor\
ary_dir (\"set\",$infile,\"seq.pep\");\n	%s=read_f\
asta_seq (\"seq.pep\");\n	\n	%pdb_template_h = &re\
ad_template_file($pdbfile);\n	my $pdb_chain;\n	ope\
n (R, \">result.aln\");\n\n\n	#print stdout \"\\n\\
";\n	foreach $seq (keys(%s))\n	{\n		if ($pdb_templ\
ate_h{$seq} eq \"\")\n		{\n			next;\n		}\n		open (\
F, \">seqfile\");\n		print (F \">$s{$seq}{name}\\n\
...
ame);\n		\n 		safe_system (\"secondary_struc.py se\
qfile $CACHE$pdb_chain  $lib_name\");\n		\n		if ( \
!-e $lib_name)\n		{\n		myexit(flush_error(\"RNAplf\
old failed to compute the secondary structure of $\
s{$seq}{name}\"));\n			myexit ($EXIT_FAILURE);\n		\
...
,$outfile, @profiles);\n}\n\n\n\nsub seq2rna_pair{\
\n	my ($mode, $pdbfile1, $pdbfile2, $method, $para\
...
n		$tmp_file=&vtmpnam();\n	\n		safe_system(\"runsa\
ra.py $pdbfile1 $chain1 $pdbfile2 $chain2 -s -o $t\
mp_file --limitation 5000 > /dev/null 2> /dev/null\
\") == 0 or die \"sara did not work $!\\n\";\n		op\
en(TMP,\"<$tmp_file\") or die \"cannot open the sa\
ra tmp file:$!\\n\";\n		open(OUT,\">$outfile\") or\
 die \"cannot open the $outfile file:$!\\n\";\n\n	\
	my $switch = 0;\n		my $seqNum = 0;\n		foreach my \
$line (<TMP>)\n		{\n			next unless ($line=~/SARAAL\
I/);\n			if ($line=~/>/)\n			{\n				$switch =0;\n	\
			print OUT \">seq$seqNum\\n\";\n				$seqNum++;		\
		\n			}\n			if ($switch < 2){\n				$switch++;\n		\
		next;\n			}\n	\n			if ($line =~/REMARK\\s+SARAAL\
I\\s+([^\\*]+)\\*/)\n			{\n				my $string = $1;\n	\
			print OUT \"$string\\n\";\n			}\n		}\n		close T\
MP; \n		close OUT;\n		unlink($tmp_file);\n	}\n}\n\\
nsub seq2tblastx_lib\n  {\n    my ($mode, $infile,\
 $outfile)=@_;\n    my (%s, $method,$nseq);\n\n   \
 $method=$mode;\n    &set_temporary_dir (\"set\",$\
infile,\"infile\");\n    %s=read_fasta_seq(\"infil\
e\");\n    \n    \n    foreach $seq (keys(%s))\n  \
...
blastall -p tblastx -i infile -d infile -m 7 -S1>b\
last.output\");\n    \n    ncbi_tblastx_xml2lib_fi\
le (\"outfile\", file2string (\"blast.output\"));\\
...
q(\"infile\");\n    \n    foreach $seq (keys(%s))\\
n      {\n	$slist[$s{$seq}{order}]=$s{$seq}{seq};\\
n	$sname[$s{$seq}{order}]=$s{$seq}{name};\n	$slen[\
$s{$seq}{order}]=length ($s{$seq}{seq});\n      }\\
n    $nseq=$#sname+1;\n    open (F, \">outfile\");\
\n    print F \"! TC_LIB_FORMAT_01\\n\";\n    prin\
t F \"$nseq\\n\";\n    for ($a=0; $a<$nseq;$a++)\n\
      {\n	print F \"$sname[$a] $slen[$a]  $slist[$\
a]\\n\"\n      }\n    close (F);\n    &safe_system\
(\"t_coffee -other_pg seq_reformat -in infile -out\
put tblastx_db1 > tblastxdb\");\n    &safe_system \
(\"formatdb -i tblastxdb -p T\");\n    &safe_syste\
m (\"blastall -p blastp -i tblastxdb -d tblastxdb \
-m7 >blast.output\");\n    ncbi_tblastpx_xml2lib_f\
ile (\"outfile\", file2string (\"blast.output\"), \
%s);\n    &set_temporary_dir (\"unset\",$mode, $me\
thod, \"outfile\",$outfile);\n    myexit ($EXIT_SU\
CCESS);\n    }\n\n\n    \n\n\n\nsub file2head\n   \
   {\n	my $file = shift;\n	my $size = shift;\n	my \
$f= new FileHandle;\n	my $line;\n	open ($f,$file);\
\n	read ($f,$line, $size);\n	close ($f);\n	return \
...
eanup();\n  }\n   \nsub blast_com2new_blast_com\n \
   {\n      my $com=shift;\n      if ($ENV{\"NCBI_\
BLAST_4_TCOFFEE\"} eq \"OLD\"){return $com;}\n    \
  elsif (!&pg_is_installed(\"legacy_blast.pl\")){r\
eturn $com;}\n      else \n	{\n	  if ($com=~/forma\
...
turn $com;\n	    }\n	  elsif (&is_blast_package($c\
om))\n	    {\n	      my $path;\n	      \n	      if\
 ( $ENV{\"NCBI_BIN_4_TCOFFEE\"}){$path=$ENV{\"NCBI\
_BLAST_4_TCOFFEE\"};}\n	      else\n		{\n		  $path\
...
my $status;\n  my $ppid=getppid();\n  if ($com eq \
\"\"){return 1;}\n  \n  if ( ($com=~/^blast/) ||($\
com=~/^formatdb/)){$com=&blast_com2new_blast_com($\
...
UNS=2;\nour $EXIT_SUCCESS=0;\nour $EXIT_FAILURE=1;\
\nour $CONFIGURATION=-1;\nour $REF_EMAIL=\"\";\nou\
r $PROGRAM=\"extract_from_pdb\";\n\n\nmy %onelett_\
prot=&fill_onelett_prot();\nmy %threelett_prot=&fi\
ll_threelett_prot();\nmy %onelett_RNA=&fill_onelet\
t_RNA();\nmy %threelett_RNA=&fill_threelett_RNA();\
\nmy %onelett_DNA=&fill_onelett_DNA();\nmy %threel\
ett_DNA=&fill_threelett_DNA();\n\n\n\n\n\nmy %onel\
ett = (\n'P' => \\%onelett_prot,\n'D' => \\%onelet\
t_DNA,\n'R' => \\%onelett_RNA\n);\n\n\nmy %threele\
tt = (\n'P' => \\%threelett_prot,\n'D' => \\%three\
lett_DNA,\n'R' => \\%threelett_RNA\n);\n\n\n\n\n\n\
...
ct_from_pdb Version $VersionTag	\n	Minimum:       \
 ...
 [... | extract_from_pdb]\n 	Flags (Default settin\
g on the first line)\n	   -version................\
...[Returns the Version Number]\n           -force\
.....................[Forces the file to be treate\
d like a PDB file]\n                              \
        [Regenerates the header and SEQRES fields]\
\n           -force_name................[Forces th\
e file to be named after name]]\n           -infil\
e.....file...........[Flag can be omited]\n			    \
          [File must be pdb or fro pgm]\n         \
                             [File can also be com\
pressed Z or gz]\n                                \
      [In the case of a compressed file, you can o\
mit the gz|Z extension]\n           -netfile......\
.............[File will be fetch from the net usin\
g wget]\n                                      [wg\
et or curl must be installed]\n                   \
                   [ftp://ftp.gnu.org/pub/gnu/wget\
/]\n                                      [http://\
curl.haxx.se/]\n                                  \
    [Must also be used to retrieve the file from a\
 local pdb copy (cf netaddress)]\n           -neta\
ddress................[Address used for the retrie\
ving the netfile]\n                               \
...
ssion............[Extension if the netfile comes c\
ompressed]\n                                      \
[gz]\n           -pdb_dir...................[addre\
ss of the repertory where the pdb is installed]\n \
                                     [Supports sta\
ndard ftp style installation OR every stru in DIR]\
\n                                      [Give the \
..../pdb/structure/ dir]\n                        \
              [If value omitted, the pg gets it fr\
om the env variable PDB_DIR]\n           -netcompr\
ession_pg.........[gunzip]\n           -is_pdb_nam\
e..........name.[Returns 1 if the name is a PDB ID\
, 0 otherwise]\n           -model_type...........n\
ame.[Returns the model type if valid PDB name]\n  \
         -is_released_pdb_name name.[Returns 1 if \
the name corresponds to a released PDB file]\n    \
       -get_pdb_chains.....name...[Returns the lis\
t of chains corresponding to the entry]\n         \
  -get_pdb_id.........name...[Returns the PDB id w\
ithin the provided pdb file]\n           -get_fugu\
e_name.....name...[Turns a name into a name valid \
for fugue]\n                                      \
[Uses the netaddress to do so]\n	   -chain......FI\
RST..........[Extract the first chain only]\n		   \
    A B C..........[Extract Several chains if need\
ed]\n		       ALL............[Extract all the chai\
ns]	\n           -ligand.....ALL............[Extra\
ct the ligands in the chain (HETATM)]\n           \
            <name1>,<name2>[Extract All the named \
lignds]\n	   -ligand_only...............[Extract o\
nly the ligands]\n           -ligand_list.........\
......[Extract the list of ligands]\n	   -coor....\
...<start>..<end>.[Coordinates of the fragment to \
extract]\n			              [Omit end to include th\
e Cter]\n           -num........absolute.......[ab\
solute: relative to the seq] \n                   \
    file...........[file: relative to file]\n     \
      -num_out....new............[new: start 1->L]\
\n                       old............[old: keep\
 the file coordinates]\n           -delete.....<st\
art>..<end>.[Delete from residue start to residue \
end]\n	   -atom.......CA.............[Atoms to inc\
lude, ALL for all of them]\n		       CA O N.......\
..[Indicate several atoms if needed]\n	   -code...\
....3..............[Use the 1 letter code or the 3\
 letters code]\n	   -mode.......raw............[Ou\
tput original pdb file]\n                       pd\
b............[Output something that looks like pdb\
]\n		       fasta..........[Output the sequences i\
n fasta format]\n		       simple.........[Output a\
 format easy to parse in C ]\n            -seq_fie\
ld..ATOM...........[Field used to extract the sequ\
ence]\n		       SEQRES.........[Use the complete s\
equence]\n	   -seq.......................[Equivale\
nt to  -mode fasta]\n	   -model......1............\
..[Chosen Model in an NMR file]\n           -nodia\
gnostic..............[Switches Error Messages off]\
\n           -debug.....................[Sets the \
DEBUG ON]\n           -no_remote_pdb_dir.........[\
Do not look for a remote file]\n           -cache_\
pdb.................[Cache Value, default is $HOME\
/.t_coffee/cache, other values: NO<=> No cache]\n\\
n      Environement Variables\n           These va\
riables can be set from the environement\n        \
   Command line values with the corresponding flag\
 superseed evironement value\n           NO_REMOTE\
_PDB_DIR..........[Prevents the program from searc\
hing remote file: faster]\n           PDB_DIR.....\
...............[Indicates where PDB file must be f\
etched (localy)]\n\n	 PROBLEMS: please contact ced\
ric.notredame\\@europe.com\\n\";\n	 exit ($EXIT_SU\
CCESS);\n}\n\n$np=0;\n$n_para=$#ARGV;\n$model=1;\n\
...
[$np];\n      }\n    elsif ( !($value=~/^-.*/))\n \
     {\n	print \"@ARGV\";\n	die;\n      } \n    \n\
    elsif ($value eq \"-nodiagnostic\"){$nodiagnos\
...
R \"\\n[DEBUG: extract_from_pdb] Scan For $local_p\
db_file\\n\";}\n\n$mem=$no_remote_pdb_dir;\n$no_re\
mote_pdb_dir=1;\n$tmp_pdb_file=get_pdb_file ($loca\
l_pdb_file);\n\nif ( !-e $tmp_pdb_file || $tmp_pdb\
_file eq \"\")\n  {\n    $local_pdb_file=$pdb_file\
;\n    ($local_pdb_file, $suffix_chain)=&pdb_name2\
name_and_chain($local_pdb_file);\n\n    if ($local\
_pdb_file)\n      {\n	if ( $debug){print STDERR \"\
\\nSplit $pdb_file into $local_pdb_file and $suffi\
...
cal_pdb_file);\n	if ($local_pdb_file)\n	  {\n	    \
\n	    if ( $debug){print STDERR \"\\nSplit $pdb_f\
ile into $local_pdb_file and $suffix_chain \\n\";}\
\n	    \n	    $tmp_pdb_file=get_pdb_file ($local_p\
db_file);    \n	    \n	    if ( $tmp_pdb_file ne \\
"\")\n	      {\n		@c_chain=();\n		@c_chain=($suffi\
x_chain);\n		%hc_chain=();\n		$hc_chain{$suffix_ch\
ain}=1;\n	      }\n	  }\n      }\n  }\n\nif ( $deb\
ug){print STDERR \"\\n$pdb_file copied into ##$tmp\
_pdb_file##\\n\";}\n\nif ( !-e $tmp_pdb_file || $t\
mp_pdb_file eq \"\")\n{\n	if ($is_pdb_name)\n{\n	 \
   print \"0\\n\"; exit ($EXIT_SUCCESS);\n}\n	else\
\n{\n  \n	    print \"\\nEXTRACT_FROM_PDB: NO RESU\
LT for $pdb_file\\n\";\n	    &myexit ($EXIT_SUCCES\
S);	\n}\n}\n\n\n\n\n%molecule_type=&pdbfile2chaint\
ype($tmp_pdb_file);\nif ( $debug){print STDERR \"\\
\n\\tSequence Type determined\\n\";}\n\n$pdb_id=&g\
et_pdb_id ($tmp_pdb_file);\nif ( $debug){print STD\
ERR \"\\n\\tID: $pdb_id (for $tmp_pdb_file)\\n\";}\
\n\nif ( $pdb_id eq \"\"){$pdb_id=$force_name;}\n\\
n@f_chain=&get_chain_list ($tmp_pdb_file);\nif ( $\
debug){print STDERR \"\\n\\tChain_list:@f_chain\\n\
\";}\n\nif ( $get_pdb_chains)\n{\n    print \"@f_c\
hain\\n\";\n    &myexit ($EXIT_SUCCESS);\n}\nif ( \
$get_pdb_ligands)\n{\n    %complete_ligand_list=&g\
et_ligand_list ($tmp_pdb_file);\n    print $comple\
te_ligand_list{\"result\"};\n    &myexit ($EXIT_SU\
CCESS);\n}\n\nelsif ( $get_pdb_id ||$get_fugue_nam\
e )\n{\n    if    (@c_chain && $c_chain[0] eq \"FI\
RST\"){$pdb_id=$pdb_id.$f_chain[0];}\n    elsif (@\
...
=\"new\";}\n\n  if ( $delete_set && $coor_set) {di\
e \"-delete and -coor are mutually exclusive, sorr\
y\\n\";}\n  if ( $n_atom==0){$atom_list[$n_atom++]\
=\"ALL\";$atom_list{$atom_list[0]}=1;}\n  if ( $se\
q_field eq \"\"){$seq_field=\"ATOM\";}\n  \n  if (\
 $MODE eq \"\"){$MODE=\"pdb\";}\n  elsif ( $MODE e\
q \"simple\" && $code==0){$code=1;}\n\n  if ( $cod\
e==0){$code=3;}\n\n\nif ($f_chain[0] eq \" \"){$hc\
_chain{' '}=1;$c_chain[0]=\" \";}\nelsif (!@c_chai\
n){$hc_chain{FIRST}=1;$c_chain[0]=\"FIRST\";}#make\
 sure the first chain is taken by default\n\nif   \
 ($hc_chain{ALL}) \n{\n      @c_chain=@f_chain;\n \
     foreach $e (@c_chain){$hc_chain{$e}=1;}\n}\ne\
lsif($hc_chain{FIRST})\n{\n	@c_chain=($f_chain[0])\
;\n	$hc_chain{$f_chain[0]}=1;\n}\n\n\n$MAIN_HOM_CO\
DE=&get_main_hom_code ($structure_file);\n$INFILE=\
vfopen ($structure_file, \"r\");\n\n\nif ( $MODE e\
q \"raw_pdb\" || $MODE eq \"raw\")\n{\n    while (\
<$INFILE>)\n{	print \"$_\";}\n    close ( $INFILE)\
;\n    &myexit($EXIT_SUCCESS);\n}    \nif ( $MODE \
eq \"raw4fugue\" )\n{\n    while (<$INFILE>)\n{	\n\
	$l=$_;\n	if ($l=~/^SEQRES/)\n{\n	    \n	    $c= s\
...
t eq \" \"){print \" NULL;\\n\";}\n    else \n{\n \
     print \" $last;\\n\";\n}\n    @c_chain=(@c_ch\
ain, $last);\n    \n    print \"REMARK Output of t\
he program extract_from_pdb (Version $VersionTag)\\
\n\";\n    print \"REMARK Legal PDB format not Gua\
ranteed\\n\";\n    print \"REMARK This format is n\
ot meant to be used in place of the PDB format\\n\\
";\n    print \"REMARK The header refers to the or\
iginal entry\\n\";\n    print \"REMARK The sequenc\
e from the original file has been taken in the fie\
ld: $seq_field\\n\";\n    print \"REMARK extract_f\
rom_pdb, 2001, 2002, 2003, 2004, 2005 2006 (c) CNR\
S and Cedric Notredame\\n\";   \n    if ( $coor_se\
...
elsif ( $seq_field eq \"ATOM\")  {@pdb_seq=@{$atom\
_seq{$c}};}\n	\n\n	$full_length=$l=$#pdb_seq+1;\n	\
	\n	if ( $real_end{$c}==\"*\"){$real_end{$c}=$full\
_length;}\n	if ( $coor_set)\n{	   \n\n	   if ( $re\
al_end{$c} < $l){splice @pdb_seq, $real_end{$c}, $\
l;}\n	   if ( $real_start{$c} < $l){splice @pdb_se\
q, 0, $real_start{$c}-1;}	  	   \n	   $l=$#pdb_seq\
;\n}\n\n	elsif ( $delete_set)\n{\n	   splice @pdb_\
seq, $delete_start, $delete_end-$delete_start+1;\n\
	   $l=$#pdb_seq;\n}\n	\n	$new_fasta_name=\"$pdb_i\
d$c\";\n	if ( $coor_set)\n{\n	   if ( $n_pdb_chain\
s==0){$new_fasta_name=\"$new_fasta_name$c\";}\n	  \
 $new_fasta_name= $new_fasta_name.\"\\_$start\\_$e\
nd\";\n}\n	   \n	if ( $MODE eq \"pdb\")\n{\n	   $n\
l=1;\n	   $n=0;\n	   \n	   foreach $res ( @pdb_seq\
)\n		{\n		if ( !$n)\n		{\n		\n		 printf \"SEQRES %\
3d %1s %4d  \", $nl,$c, $l;\n		 $nl++;\n	}\n	     \
$res=~s/\\s//g;\n	     \n	     if ($code==1){ prin\
tf \"%3s \",$onelett{$molecule_type{$c}}->{$res};}\
\n	     elsif  ($code==3){ printf \"%3s \",$res};\\
n	     \n	     $n++;		  \n	     if ( $n==13){$n=0;\
print \"\\n\";}\n}\n	  if ( $n!=0){print \"\\n\"; \
$n=0;}\n}\n	elsif ( $MODE eq \"simple\")\n{\n	  pr\
int \"# SIMPLE_PDB_FORMAT\\n\";\n	  if ( $new_fast\
a_name eq \" \"){$new_fasta_name=\"dummy_name\";}\\
n	  print \">$new_fasta_name\\n\";\n\n	  foreach $\
res ( @pdb_seq)\n{\n	      print \"$onelett{$molec\
ule_type{$c}}->{$res}\";\n}\n	  print \"\\n\";\n}\\
n	elsif ( $MODE eq \"fasta\")\n{\n	  $n=0;\n	  pri\
nt \">$new_fasta_name\\n\";\n	  \n	  foreach $res \
...
is_aa($RES_ID,$chain))?&is_aa($RES_ID,$chain):$RES\
_ID;\n}\n	elsif ($line=~/^HETATM/ && ($ligand_list\
 {$RES_ID} ||$ligand_list {'ALL'} || !&is_aa($RES_\
ID,$chain)))\n{\n	    $line_header=\"HETATM\";\n}\\
n	elsif ($line=~/^HETATM/ && (&is_aa($RES_ID,$chai\
n) && !$no_hetatm))\n{\n	    $line_header=\"ATOM\"\
...
eld eq \"SEQRES\"){@ref_seq=@{$complete_seq{$CHAIN\
}};}\n	    elsif ( $seq_field eq \"ATOM\")  {@ref_\
seq=@{$atom_seq{$CHAIN}};}\n}\n	\n	if ($current_re\
sidue != $RES_NO)\n{\n	    $current_residue=$RES_N\
O;\n	    if    ( $seq_field eq \"SEQRES\"){$pos=$c\
urrent_residue;}\n	    elsif ( $seq_field eq \"ATO\
M\"){$pos++;}\n}\n	\n	\n	if ($n_atom==0 || $atom_l\
ist{$AT_ID}==1 || $atom_list{$KEY}==1)\n{ 	\n	    \
\n	    $do_it=(!@c_chain || $hc_chain{$CHAIN} ||$h\
c_chain{'LIGAND'} );\n	    \n	    $do_it= ($do_it=\
=1) && ($coor_set==0 ||($pos>=$real_start{$CHAIN} \
&& $pos<=$real_end{$CHAIN}));\n	    $do_it= ($do_i\
t==1) && ($delete_set==0 || $pos<$delete_start ||$\
pos>$delete_end );\n	    if ($ligand==0 && $line_h\
eader eq \"HETATM\" ){$do_it=0;}\n	    if ($ligand\
_only==1 && $line_header eq \"ATOM\" ){$do_it=0;}\\
n	    if ($ligand==1 && $line_header eq \"HETATM\"\
...
5;\n		   $X=$unfolded_x;\n		   $Y=0;\n		   $Z=0;\n\
		   $float=1;\n}\n	       else\n{\n		   $float=3;\
\n}\n\n	       if ( $MODE eq \"pdb\")\n{\n		   pri\
ntf \"%-6s%5d %-4s %3s %s%4d    %8.3f%8.3f%8.3f  1\
.00 %5.2f\\n\",$line_header, $n, $RAW_AT_ID,$RES_I\
D,$CHAIN,$out_pos, $X, $Y, $Z,$TEMP;		  \n}\n	    \
   elsif ( $MODE eq \"simple\")\n{\n		    if ( $RE\
S_ID eq \"\"){$RES_ID=\"X\";}\n		  printf \"%-6s %\
...
\n}\n}\n}\n}\nprint \"\\n\";\nclose($INFILE);\n\n\\
nif ( $error ne \"\") \n{$error=$error.\"\\nDiagno\
stic:    SEQRES and the residues in ATOM are proba\
bly Incompatible\\n\";\n    $error=$error.  \"Reco\
mendation: Rerun with '-fix 1' in order to ignore \
the SEQRES sequences\\n\";\n}\nif (!$nodiagnostic)\
{print STDERR $error;}\n&myexit ( $EXIT_SUCCESS);\\
n\nsub is_released \n  {\n    my ($r);\n    my $in\
...
r=($name && !$hold)?1:0;\n    return $r;\n  }\nsub\
 remote_is_pdb_name \n{\n    my $in=@_[0];\n    my\
 ($ref_file, $pdb);\n    my ($value,$value1,$value\
2);\n\n    if ( $in=~/[^\\w\\d\\:\\_]/){return 0;}\
\n    $ref_file=\"$cache/pdb_entry_type.txt\";\n  \
  \n    if ( !-e $ref_file || (-M $ref_file)>2 || \
-z $ref_file)\n      {\n	&url2file(\"ftp://ftp.wwp\
db.org/pub/pdb/derived_data/pdb_entry_type.txt\", \
$ref_file);\n      }\n    $pdb=substr ($in,0, 4);\\
...
0];\n    my ($ref_file, $pdb);\n    my ($value1, $\
value2,$value);\n    \n    if ( $in=~/[^\\w\\d\\:\\
\_]/){return 0;}\n    $ref_file=\"$cache/unrelease\
d.xml\";\n    \n    if ( !-e $ref_file || (-M $ref\
_file)>2 || -z $ref_file)\n      {\n	&url2file(\"h\
ttp://www.rcsb.org/pdb/rest/getUnreleased\",$ref_f\
ile);\n      }\n    \n    $pdb=substr ($in,0, 4);\\
n    chomp(($value1=`grep -c $pdb $ref_file`));\n \
   $pdb=lc($pdb);\n    chomp(($value2=`grep -c $pd\
b $ref_file`));\n    $value=($value1 || $value2)?1\
...
return \"\";\n}\n\nsub get_ligand_list\n{\n    my \
$pdb_file=@_[0];\n    my $chain;\n    my $ligand;\\
n    my %complete_ligand_list;\n    \n\n    $F=vfo\
pen ($pdb_file, \"r\");\n    while ( <$F>)\n{\n	if\
 ( /^HETATM/)\n{\n	    $line=$_;\n	    $chain=subs\
tr($line,21,1);\n	    $ligand=substr($line,17,3);\\
n	    \n	    if (!$complete_ligand_list{$chain}{$l\
igand})\n{\n		\n		$complete_ligand_list{\"result\"\
}.=\"CHAIN $chain LIGAND $ligand\\n\";\n		$complet\
e_ligand_list{$chain}{$ligand}=1;\n}\n}\n}\n    cl\
...
;if ( $chain eq \"\"){$chain=\" \";}\n	    if (!$h\
asch{$chain}){$hasch{$chain}=1;push @chain_list, $\
chain;}\n	  }\n	if (/^ATOM/ || /^HETATM/)\n	  {\n	\
    $chain = substr($_,21,1); $chain=~s/\\s//g;if \
( $chain eq \"\"){$chain=\" \";}\n	    if (!$hasch\
{$chain}){$hasch{$chain}=1;push @chain_list, $chai\
n;}\n	  }\n      }\n\n\nclose ($F);\nif (!@chain_l\
ist)\n  {\n    @chain_list=(\"A\");\n  }\n\n\nretu\
...
$pdb_file=($pdb_file_in=~/\\S+_S_(\\S+)/)?$1:$pdb_\
file_in;\n    \n    if ($no_remote_pdb_dir==0)\n  \
    {\n	$no_remote_pdb_dir=1;\n	$result=get_pdb_fi\
le3 ($pdb_file);\n	$no_remote_pdb_dir=0;\n	if ( $r\
esult){return $result;}\n	else\n	  {\n	    \n	    \
...
file;\n    my $pdb_file=$pdb_file_in;\n    \n    $\
lcfile=lc $pdb_file;\n    $ucfile=uc $pdb_file;\n\\
n    if ( ($result=get_pdb_file2 ($pdb_file))){ret\
urn $result;}\n    \n\n    if ($lcfile ne $pdb_fil\
e && ($result=get_pdb_file2 ($lcfile))){return $re\
sult;}\n    if ($ucfile ne $pdb_file && ($result=g\
et_pdb_file2 ($ucfile))){return $result;}\n    \n \
  \n    \n    return \"\";\n}\nsub get_pdb_file2\n\
{\n    my $pdb_file=@_[0];\n    my $return_value;\\
n    \n    $return_value=\"\";\n    \n    if ( ($r\
esult=get_pdb_file1 ($pdb_file))){$return_value=$r\
esult;}\n    elsif ( !($pdb_file=~/\\.pdb/) && !($\
pdb_file=~/\\.PDB/))\n{\n	if ( ($result=get_pdb_fi\
le1 (\"$pdb_file.pdb\"))){$return_value=$result;}\\
n	elsif ( ($result=get_pdb_file1 (\"$pdb_file.PDB\\
"))){$return_value=$result;}\n\n	elsif ( ($result=\
get_pdb_file1 (\"pdb$pdb_file.pdb\"))){$return_val\
ue=$result;}	\n	elsif ( ($result=get_pdb_file1 (\"\
pdb$pdb_file.PDB\"))){$return_value=$result;}\n	el\
sif ( ($result=get_pdb_file1 (\"PDB$pdb_file.PDB\"\
))){$return_value=$result;}\n	elsif ( ($result=get\
...
($result=get_pdb_file1 (\"pdb$pdb_file.ENT\"))){$r\
...
et_pdb_file0 (\"$pdb_file.Z\"))){$return_value=$re\
sult;}\n    elsif ( ($result=get_pdb_file0 (\"$pdb\
_file.gz\"))){$return_value=$result;}\n    elsif (\
 ($result=get_pdb_file0 (\"$pdb_file.GZ\"))){$retu\
rn_value=$result;}\n    return $return_value;\n}\n\
sub get_pdb_file0 \n{ \n    my ($pdb_file, $attemp\
t)=(@_);\n    my $pdb_file=@_[0];\n    my $tmp_pdb\
_file;    \n    my $return_value;\n\n    if ( !$at\
...
( $local_pdb_file))\n{return $tmp_pdb_file;}\n}\n\\
n    $local_pdb_file=\"$pdb_file\";\n    &debug_pr\
int (\"\\nTry access local file: $local_pdb_file\"\
);\n    \n    $local_pdb_file=&check_pdb_file4comp\
ression ($local_pdb_file);\n    if ( -e $local_pdb\
_file && (&is_pdb_file ($local_pdb_file) || $force\
_pdb))\n{\n	&debug_print ( \"\\n\\tIs in Current D\
ir\");\n	$tmp_pdb_file=vtmpnam();\n	`cp $local_pdb\
_file $tmp_pdb_file`;\n	return $tmp_pdb_file;\n}\n\
    else\n{\n	&debug_print (\"\\n\\tFile Not in Cu\
rrent Dir\");\n}\n\n    if ($pdb_file=~/^pdb/||$pd\
b_file=~/^PDB/){$pdb_div=substr ($pdb_file, 4, 2);\
}\n    else\n{\n	  $pdb_div=substr ($pdb_file, 1, \
2);\n}\n    $local_pdb_file=\"$pdb_dir/$pdb_div/$p\
db_file\";\n    $local_pdb_file=&check_pdb_file4co\
mpression ( $local_pdb_file);\n    &debug_print (\\
"\\nTry access file From PDB_DIR: $local_pdb_file\\
");\n    if ($pdb_dir && -e $local_pdb_file && &is\
...
_print (\"\\nTry access file From PDB_DIR: local_p\
db_file\");\n    if ($pdb_dir && -e $local_pdb_fil\
e && &is_pdb_file ($local_pdb_file))\n{\n	&debug_p\
rint ( \"\\n\\tIs in Local PDB DIR\");\n	$tmp_pdb_\
file=vtmpnam();\n	`cp $local_pdb_file $tmp_pdb_fil\
e`;\n	return $tmp_pdb_file;\n}\n\n    $local_pdb_f\
ile=\"$pdb_dir$pdb_file\";\n    $local_pdb_file=&c\
...
le && &is_pdb_file ($local_pdb_file))\n{\n	    &de\
bug_print ( \"\\n\\tIs in T-Coffee Cache\");\n	   \
 $tmp_pdb_file=vtmpnam();\n	    `cp $local_pdb_fil\
e $tmp_pdb_file`;\n	    return $tmp_pdb_file;\n}\n\
	else{&debug_print ( \"\\n\\tNot in Cache Dir\");}\
\n}\n\nif (!$no_remote_pdb_dir) \n  {\n    my $val\
ue=&is_released ($pdb_file);\n    my $return_value\
=\"\";\n    if ($value==1)\n      {\n	\n	&debug_pr\
int (\"\\n****************************************\
...
");\n	&debug_print(\"\\nREMOTE: $netcommand\\n\");\
\n	\n	$compressed_tmp_file_name=\"$tmp_pdb_file.$n\
etcompression\";\n	\n	if ($netcompression && -B $c\
ompressed_tmp_file_name)\n	  {\n	    my $r;\n	    \
&debug_print (\"\\n\\tFile Found Remotely\");\n	  \
  if (($r=safe_system ( \"$netcompression_pg $comp\
ressed_tmp_file_name\")!=$EXIT_SUCCESS) && $attemp\
ts<5)\n	      {\n		&debug_print (\"\\n\\tProper Do\
wnload Failed Try again\");\n		unlink $compressed_\
tmp_file_name;\n		print \"\\nFailed to Download $c\
ompressed_tmp_file_name. New Attempt $attempt/5\\n\
...
@_[1];\n    my $tmp;\n    my $F = new FileHandle;\\
n    \n    \n    $tmp=$file;\n	\n    \n    if ( $m\
ode eq \"r\" && !-e $file){ myexit(flush_error (\"\
...
is_aa \n{\n    my ($aa, $chain) =@_;\n\n    my $on\
e;\n    my $trhee;\n    \n    if ( $onelett{$molec\
ule_type{$chain}}->{$aa} eq 'X' || !$onelett{$mole\
cule_type{$chain}}->{$aa} ){return '';}\n    else\\
...
$wget_arg, $curl_arg)=(@_);\n    my ($pg, $flag, $\
r, $arg, $count);\n    \n    if (!$CONFIGURATION){\
&check_configuration (\"wget\", \"INTERNET\", \"gz\
ip\");$CONFIGURATION=1;}\n    \n    if (&pg_is_ins\
talled (\"wget\"))   {$pg=\"wget\"; $flag=\"-O\";$\
...
\";}\n		else \n		  {\n		    myexit(flush_error(\"E\
RROR: Could not read RES_ID field in file $file\")\
);\n		  }\n	      }\n	  }\n      }\n    close ($F)\
;\n    return %ct;\n  }\n   \n    \n\n\n\nsub fill\
_threelett_RNA\n{\n\n	my %threelett=(\n	'A', '  A'\
,\n	'T', '  T',\n	'U', '  U',\n	'C', '  C',\n	'G',\
...
n  }\nsub isshellpid\n  {\n    my $p=shift;\n    i\
f (!lock4tc ($p, \"LLOCK\", \"LCHECK\")){return 0;\
}\n    else\n      {\n	my $c=lock4tc($p, \"LLOCK\"\
, \"LREAD\");\n	if ( $c=~/-SHELL-/){return 1;}\n  \
    }\n    return 0;\n  }\nsub isrootpid\n  {\n   \
 if(lock4tc (getppid(), \"LLOCK\", \"LCHECK\")){re\
turn 0;}\n    else {return 1;}\n  }\nsub lock4tc\n\
	{\n	  my ($pid,$type,$action,$value)=@_;\n	  my $\
fname;\n	  my $host=hostname;\n	  \n	  if ($type e\
q \"LLOCK\"){$fname=\"$LOCKDIR/.$pid.$host.lock4tc\
offee\";}\n	  elsif ( $type eq \"LERROR\"){ $fname\
=\"$LOCKDIR/.$pid.$host.error4tcoffee\";}\n	  elsi\
f ( $type eq \"LWARNING\"){ $fname=\"$LOCKDIR/.$pi\
d.$host.warning4tcoffee\";}\n	  \n	  if ($debug_lo\
ck)\n	    {\n	      print STDERR \"\\n\\t---lock4t\
c(tcg): $action => $fname =>$value (RD: $LOCKDIR)\\
\n\";\n	    }\n\n	  if    ($action eq \"LCHECK\") \
{return -e $fname;}\n	  elsif ($action eq \"LREAD\\
"){return file2string($fname);}\n	  elsif ($action\
 eq \"LSET\") {return string2file ($value, $fname,\
 \">>\");}\n	  elsif ($action eq \"LRESET\") {retu\
...
TRAP'}='signal_cleanup';\n      $SIG{'SIGABRT'}='s\
ignal_cleanup';\n      $SIG{'SIGEMT'}='signal_clea\
nup';\n      $SIG{'SIGFPE'}='signal_cleanup';\n   \
   \n      $SIG{'SIGKILL'}='signal_cleanup';\n    \
  $SIG{'SIGPIPE'}='signal_cleanup';\n      $SIG{'S\
IGSTOP'}='signal_cleanup';\n      $SIG{'SIGTTIN'}=\
'signal_cleanup';\n      $SIG{'SIGXFSZ'}='signal_c\
leanup';\n      $SIG{'SIGINFO'}='signal_cleanup';\\
n      \n      $SIG{'SIGBUS'}='signal_cleanup';\n \
     $SIG{'SIGALRM'}='signal_cleanup';\n      $SIG\
{'SIGTSTP'}='signal_cleanup';\n      $SIG{'SIGTTOU\
'}='signal_cleanup';\n      $SIG{'SIGVTALRM'}='sig\
nal_cleanup';\n      $SIG{'SIGUSR1'}='signal_clean\
up';\n\n\n      $SIG{'SIGSEGV'}='signal_cleanup';\\
n      $SIG{'SIGTERM'}='signal_cleanup';\n      $S\
IG{'SIGCONT'}='signal_cleanup';\n      $SIG{'SIGIO\
'}='signal_cleanup';\n      $SIG{'SIGPROF'}='signa\
l_cleanup';\n      $SIG{'SIGUSR2'}='signal_cleanup\
';\n\n      $SIG{'SIGSYS'}='signal_cleanup';\n    \
  $SIG{'SIGURG'}='signal_cleanup';\n      $SIG{'SI\
GCHLD'}='signal_cleanup';\n      $SIG{'SIGXCPU'}='\
signal_cleanup';\n      $SIG{'SIGWINCH'}='signal_c\
leanup';\n      \n      $SIG{'INT'}='signal_cleanu\
...
n\";}\n      if ( $debug_lock ){print STDERR \"\\n\
 --- $$ -- $CL\\n\";}\n      \n	     \n      \n   \
   \n    }\nsub flush_error\n  {\n    my $msg=shif\
t;\n    return add_error ($EXIT_FAILURE,$$, $$,get\
ppid(), $msg, $CL);\n  }\nsub add_error \n  {\n   \
 my $code=shift;\n    my $rpid=shift;\n    my $pid\
=shift;\n    my $ppid=shift;\n    my $type=shift;\\
n    my $com=shift;\n    \n    $ERROR_DONE=1;\n   \
 lock4tc ($rpid, \"LERROR\",\"LSET\",\"$pid -- ERR\
OR: $type\\n\");\n    lock4tc ($$, \"LERROR\",\"LS\
ET\", \"$pid -- COM: $com\\n\");\n    lock4tc ($$,\
 \"LERROR\",\"LSET\", \"$pid -- STACK: $ppid -> $p\
id\\n\");\n   \n    return $code;\n  }\nsub add_wa\
rning \n  {\n    my $rpid=shift;\n    my $pid =shi\
ft;\n    my $command=shift;\n    my $msg=\"$$ -- W\
ARNING: $command\\n\";\n    print STDERR \"$msg\";\
...
\n    exit ($EXIT_FAILURE);\n  }\nsub clean_dir\n \
 {\n    my $dir=@_[0];\n    if ( !-d $dir){return \
;}\n    elsif (!($dir=~/tmp/)){return ;}#safety ch\
eck 1\n    elsif (($dir=~/\\*/)){return ;}#safety \
check 2\n    else\n      {\n	`rm -rf $dir`;\n     \
 }\n    return;\n  }\nsub cleanup\n  {\n    #print\
 stderr \"\\n----tc: $$ Kills $PIDCHILD\\n\";\n   \
 #kill (SIGTERM,$PIDCHILD);\n    my $p=getppid();\\
n    $CLEAN_EXIT_STARTED=1;\n    \n    \n    \n   \
...
se\n    {\n      lock4tc ($$, \"LLOCK\", \"LSET\",\
 \"$pid\\n\");#update parent\n      $PIDCHILD=$pid\
;\n    }\n  if ($debug_lock){printf STDERR \"\\n\\\
t .... safe_system (fasta_seq2hmm)  p: $$ c: $pid \
COM: $com\\n\";}\n\n  waitpid ($pid,WTERMSIG);\n\n\
...
$$,$$,\"$com failed [retry: $ctry]\");\n	  lock4tc\
 ($pid, \"LRELEASE\", \"LERROR\", \"\");\n	  retur\
n safe_system ($com, $ntry, ++$ctry);\n	}\n      e\
lsif ($ntry == -1)\n	{\n	  if (!shift_lock ($pid, \
$$, \"LERROR\", \"LWARNING\", \"LSET\"))\n	    {\n\
	      add_warning ($$,$$,\"$com failed\");\n	    \
}\n	  else\n	    {\n	      lock4tc ($pid, \"LRELEA\
SE\", \"LERROR\", \"\");\n	    }\n	  return $?;}\n\
      else\n	{\n	  if (!shift_lock ($pid,$$, \"LER\
ROR\",\"LERROR\", \"LSET\"))\n	    {\n	      myexi\
t(add_error ($EXIT_FAILURE,$$,$pid,getppid(), \"UN\
SPECIFIED system\", $com));\n	    }\n	}\n    }\n  \
return $?;\n}\n\nsub check_configuration \n    {\n\
      my @l=@_;\n      my $v;\n      foreach my $p\
 (@l)\n	{\n	  \n	  if   ( $p eq \"EMAIL\")\n	    {\
 \n	      if ( !($EMAIL=~/@/))\n		{\n		add_warning\
($$,$$,\"Could Not Use EMAIL\");\n		myexit(add_err\
or ($EXIT_FAILURE,$$,$$,getppid(),\"EMAIL\",\"$CL\\
"));\n	      }\n	    }\n	  elsif( $p eq \"INTERNET\
\")\n	    {\n	      if ( !&check_internet_connecti\
on())\n		{\n		  myexit(add_error ($EXIT_FAILURE,$$\
,$$,getppid(),\"INTERNET\",\"$CL\"));\n		}\n	    }\
\n	  elsif( $p eq \"wget\")\n	    {\n	      if (!&\
pg_is_installed (\"wget\") && !&pg_is_installed (\\
"curl\"))\n		{\n		  myexit(add_error ($EXIT_FAILUR\
E,$$,$$,getppid(),\"PG_NOT_INSTALLED:wget\",\"$CL\\
"));\n		}\n	    }\n	  elsif( !(&pg_is_installed ($\
p)))\n	    {\n	      myexit(add_error ($EXIT_FAILU\
RE,$$,$$,getppid(),\"PG_NOT_INSTALLED:$p\",\"$CL\"\
));\n	    }\n	}\n      return 1;\n    }\nsub pg_is\
_installed\n  {\n    my @ml=@_;\n    my $r, $p, $m\
;\n    my $supported=0;\n    \n    my $p=shift (@m\
l);\n    if ($p=~/::/)\n      {\n	if (safe_system \
...
g_is_installed    (\"curl\")){`curl www.google.com\
 -o$tmp >/dev/null 2>/dev/null`;}\n    \n    if ( \
!-e $tmp || -s $tmp < 10){$internet=0;}\n    else \
{$internet=1;}\n    if (-e $tmp){unlink $tmp;}\n\n\
    return $internet;\n  }\nsub check_pg_is_instal\
led\n  {\n    my @ml=@_;\n    my $r=&pg_is_install\
ed (@ml);\n    if (!$r && $p=~/::/)\n      {\n	pri\
...
'install $pg'\\n\";\n      }\n    elsif (!$r)\n   \
   {\n	myexit(flush_error(\"\\nProgram $p Supporte\
d but Not Installed on your system\"));\n      }\n\
    else\n      {\n	return 1;\n      }\n  }\n\n\n\\
n","use Cwd;\nuse Env;\nuse File::Path;\nuse FileH\
andle;\nuse strict;\n\n\nour (%MODE, %PG, %ENV_SET\
, %SUPPORTED_OS);\n\n\nour $EXIT_SUCCESS=0;\nour $\
