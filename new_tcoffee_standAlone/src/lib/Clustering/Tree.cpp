...
void
Tree::upgma(const Matrix<float> &dist_mat, const vector<string> &names)
{

	Matrix<float> distance_mat = dist_mat;
	int n_taxa = dist_mat.dim1();
	_n_species = n_taxa;
	vector<int> n_members(n_taxa, 1);
	int i,j,k,small,large,min,min_k=-1,min_j=-1,new_dist;
	int n_nodes=n_taxa*2-1;
	int node_pos = n_taxa;
	TreeNode** nodes = new TreeNode*[n_nodes];
	for (i=0; i<n_nodes; ++i)
	{
		if (i<n_taxa)
	...
	n_nodes=n_taxa;
	// the number of nodes to exist
	int *node_ids = new int[n_nodes];
	for (i=0; i<n_nodes; ++i)
	node_ids[i]=i;
	for (i=1; i<n_nodes; ++i)
	{
		// go through the matrix, find smallest match
		min=INT_MAX;
		for (j=0; j<n_nodes; ++j)
		{
			if (n_members[j]==0)
			...
			for (k=j+1; k<n_nodes; ++k)
			{
				if (n_members[k]==0)
					continue;
				if (distance_mat[j][k]<min)
				{
					min=distance_mat[j][k];
					min_k=k;
					min_j=j;
				}
			}
		...
		nodes[node_pos]->children.push_back(nodes[node_ids[min_k]]);
		nodes[node_pos]->children.push_back(nodes[node_ids[min_j]]);
		...
		for (j=0; j<n_nodes; ++j)
		{
			if ((n_members[j]==0) || (j==min_k) || (j==min_j))
				continue;

			new_dist = distance_mat[j][min_k]*n_members[min_k]+ distance_mat[min_j][j]*n_members[min_j];
			new_dist /= (n_members[min_k]+n_members[min_j]);
			distance_mat[small][j] = distance_mat[j][small]=new_dist;
		}
		node_ids[small]=node_pos++;
		n_members[small]+=n_members[large];
		n_members[large]=0;
	}
	_root=nodes[node_pos-1];
	delete[] nodes;
	delete[] node_ids;
	/*
