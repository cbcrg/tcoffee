...
float
hmm_forward(const HMM &hmm, Matrix<float> &ins_probs1, Matrix<float> &ins_probs2, Matrix<float> &match_probs, Matrix<float> &dp_mat, float **insert_matrices)
{
	unsigned short n_states=5;
...
float
hmm_backward(const HMM &hmm, Matrix<float> &ins_probs1, Matrix<float> &ins_probs2, Matrix<float> &match_probs, Matrix<float> &dp_mat, float **insert_matrices)
{
	unsigned short n_states=5;
	int l_seq1 = ins_probs1.dim1();
	int l_seq2 = ins_probs2.dim1();
	int i,j;
	float single_c1, single_c2;
	short k;
	const Matrix<float> &transProbs = hmm.trans_probs();
...
void
hmm2lib(const Sequence &seq1, const Sequence &seq2, const Matrix<float> &forward_mat, const Matrix<float> &backward_mat, BioTools::Utils::Library &lib, float total_probability)
{
	...
	for (i=0; i<l_seq1; ++i)
	{
		for (j=0; j<l_seq2; ++j)
		{
			if ((forward_mat[i][j] + backward_mat[i][j]) > max)
				max = forward_mat[i][j] + backward_mat[i][j];
		}
	}
	max=max+0.1*max;
	std::cout << max << endl;
	for (i=0; i<l_seq1; ++i)
	{
		for (j=0; j<l_seq2; ++j)
		{
			if ((tmp=forward_mat[i][j] + backward_mat[i][j]) > max)
				matches.push_back(hmm_match(EXP(tmp-total_probability), i,j));
		}
	}
*/
...
void
hmm2lib(const Sequence &seq1, int id1, const Sequence &seq2, int id2, const Matrix<float> &forward_mat, const Matrix<float> &backward_mat, BioTools::Utils::Library &lib, float total_probability)
{
	size_t l_seq1 = seq1.size();
	size_t l_seq2 = seq2.size();
	size_t i,j;
	...
	float min_score=matches[max2-1].match;
	i=0;
	while ((i<matches.size())&&(matches[i].match>=min_score))
	{
...
void
hmm2lib(const Alignment &aln1, const Alignment &aln2, const Matrix<float> &forward_mat, const Matrix<float> &backward_mat, BioTools::Utils::Library &lib, float total_probability)
{
...
void
all_hmm_pairs(const BioTools::Seq::SequenceSet &set, BioTools::Utils::Library &lib, Matrix<float> &dist_mat, char &seq_type)
{
		...
		for (i=0; i< n_seqs; ++i)
		{
			for (j=i+1; j<n_seqs; ++j)
			{
				if (set[i].size() > set[j].size())
				{
					...
					hmm2lib(seq1, i, seq2, j, forward_mat, backward_mat, lib, (bw_p+fw_p)/2);
				}
				else
				{
					const Sequence &seq1 = set[j];
					const Sequence &seq2 = set[i];
					fw_p = hmm_forward(seq1, seq2, hmm, forward_mat, insert_matrices);
					bw_p = hmm_backward(seq1, seq2, hmm, backward_mat, insert_matrices);
					hmm2lib(seq1, j, seq2, i, forward_mat, backward_mat, lib, (bw_p+fw_p)/2);
				}

				dist_mat[i][j] = dist_mat[j][i] = fw_p*(-1);
			}
		}
	} //omp
}
...
void
all_hmm_pairs(const vector<BioTools::Seq::Alignment*> &set, BioTools::Utils::Library &lib, Matrix<float> &dist_mat, char &seq_type, size_t start, size_t end)
{
...
//	#pragma omp parallel shared(set, n_seqs, max_len, hmm, dist_mat, lib) private(i, j)
	{
		...
		for (i=0; i<8; ++i)
			insert_matrices[i] = new float[max_len];
		Matrix<float> ins_probs1(1,1);
		Matrix<float> ins_probs2(1,1);
		Matrix<float> match_probs(1,1);

		#pragma omp for nowait
		for (i=start; i< end; ++i)
		{
			const Alignment *aln1 = set[i];
			hmm.calculate_insertion_probs(*aln1, ins_probs1);
			for (j=i+1; j<=end; ++j)
			{
				const Alignment *aln2 = set[j];
				hmm.calculate_insertion_probs(*aln2, ins_probs2);
				if (set[i]->size() > set[j]->size())
				{
					...
					hmm2lib(*aln1, *aln2, forward_mat, backward_mat, lib, (bw_p+fw_p)/2);
				}
				else
				{
					hmm.calculate_match_probs(*aln2, *aln1, match_probs);
					fw_p = hmm_forward(hmm, ins_probs2, ins_probs1, match_probs, forward_mat, insert_matrices);
					bw_p = hmm_backward(hmm, ins_probs2, ins_probs1, match_probs, backward_mat, insert_matrices);
					hmm2lib(*aln2, *aln1, forward_mat, backward_mat, lib, (bw_p+fw_p)/2);
				}

				dist_mat[i-start][j-start] = dist_mat[j-start][i-start] = fw_p*(-1); // 1-EXP(-log((fw_p+bw_p)/2*-1)); //EXP((fw_p+bw_p)/2);
			}
		}
	} //omp
}
	...
	#pragma omp parallel shared(set, n_seqs, max_len, hmm, dist_mat, lib) private(i, j)
	{
		...
		for (i=start; i< end; ++i)
		{
			for (j=i+1; j<end; ++j)
			{
				...
				fw_p = hmm_forward(seq1, seq2, hmm, forward_mat, insert_matrices);
				bw_p = hmm_backward(seq1, seq2, hmm, backward_mat, insert_matrices);
				hmm2lib(seq1, seq2, forward_mat, backward_mat, lib, (bw_p+fw_p)/2);
