...
write_seqs(string &aln_out_f, Alignment &aln)
{
	...
	size_t j;
	FILE *aln_F = my_fopen(aln_out_f.c_str(), "a");
	char c;
...
gap_trim(const Alignment &aln, double percentage, vector<size_t> &to_del)
{
	...
	// counting consecutive gaps in each row(seq)
	double *gapExten_counter = new double[aln_l];
	// counting gaps in each column
	double *gapFinal_counter = new double[aln_l];
	// counting letters in each column
	size_t *letter_counter = new size_t[aln_l];
	// counting gaps in each column
	size_t *gap_counter = new size_t[aln_l];
	for (j=0; j<aln_l; ++j){
		gap_counter[j] = 0;
		gapExten_counter[j]= 0;
		gapFinal_counter[j]=0;
		letter_counter[j]=0;	
	}
	...
	for (i=0; i<n_seqs; ++i)
	{
		...
		for (j=0; j<aln_l; ++j)
		{
			//if (seq[j]== '-'){	++gap_counter[j];		}
			if (seq[j]!= '-'){	++letter_counter[j];		}
			gapExten_counter[j]=0;
			if(j>0){
				if ((seq[j]== '-') && (seq[j-1]== '-')){ gap_counter[j]=gap_counter[j-1]+1; gapExten_counter[j]=1/(double)gap_counter[j]; }
				if ((seq[j]== '-') && (seq[j-1]!= '-')){ gap_counter[j]=1; ++gapExten_counter[j]=1; }
			}else{  if (seq[j]== '-'){   gapExten_counter[j]=1;    } }
		}
		
		for (j=0; j<aln_l; ++j)
		{
			if (seq[j]== '-')
				gapFinal_counter[j]=gapFinal_counter[j]+gapExten_counter[j];
		}
	}
	...
	for (i=0; i<n_seqs; ++i)
	{
		...
		{	
			if ((seq[j]!='-') && (gap_counter[j] != n_seqs)){
				seq_val[i].first+=( static_cast<double>(gapFinal_counter[j])/static_cast<double>(letter_counter[j]) ); // (number of gaps)/(number of non-gaps)
			}
		}
	}
	...
	double total=0;
	for (i=0; i<n_seqs; ++i){  total+=seq_val[i].first;  }
	...
	double so_far = 0;
	int seqNum_toRemove=0, seqCount=1;
	
	i=0;
	seqNum_toRemove=ceil(percentage*n_seqs); //printf("%d %d\n", n_seqs, seqNum_toRemove);
	
	while (seqCount <= seqNum_toRemove) /*so_far/total<percentage*/
	{
		so_far+=seq_val[i].first; //cout << seq_val[i].first << endl;
 ...
 */
void
gap_trim2(const Alignment &aln, double percentage, vector<size_t> &to_del)
{
	size_t aln_l = aln.size();
	size_t n_seqs = aln.n_seqs();
	size_t i,j;
 ...
 * \brief Identifies sequences which are introducing the most gaps.
 * \param aln The alignment
 * \param percentage The thresold to use (= % in sequences to be lost) 
 * \param[out] The ids of the sequences to delete
 */
void
random_trim(const Alignment &aln, double percentage, vector<size_t> &to_del)
{
	size_t n_seqs = aln.n_seqs();
	size_t i;
	vector<int> seqvector;
	int seqNum_toRemove=0, seq_count=1;
	
	for (i=0; i<n_seqs; ++i){ seqvector.push_back(i); } //sequence id

	// shuffle sequence ids
	//srand(time(NULL));
	//random_shuffle( seqvector.begin(), seqvector.end());
	
	// delete sequences until threshold is reached
	seqNum_toRemove=ceil(percentage*n_seqs);
	
	while (seq_count <= seqNum_toRemove) /*so_far/total<percentage*/
	{	
		to_del.push_back( seqvector[seq_count] );  
		printf(">%d\n",seqvector[seq_count]);
		++seq_count; 
	}
}
...
main(int argc, char *argv[])
{
	...
	("random_trim,r", po::value<bool> (&randomtrim)->default_value(false)->zero_tokens(), "Trim by removing sequences randomly.")
	;
	...
	if (!keep_f.empty())
	{
		...
		FILE *outFile=fopen(out_f.c_str(), "w+");
		fclose(outFile);
		write_seqs(out_f,keep_aln);
		vector<size_t> to_delInit;
		for (size_t i=0; i<n_keep; ++i){
			names2keep.insert(keep_aln[i].name()); 
			
			for(size_t j=0; j<alns[0].n_seqs(); ++j){
			      if( keep_aln[i].name() == alns[0][j].name().c_str() ){  to_delInit.push_back( j ); }
			}
		}
		
		if (!to_delInit.empty()){  alns[0].delete_seqs(to_delInit);  }
	}

	vector<size_t> to_del;
	if (n_alns > 1){    disagree_trim(alns, n_alns, threshold, to_del);    }
	else{
		if(randomtrim==false){  gap_trim(alns[0], threshold, to_del);   }
		else{  random_trim(alns[0], threshold, to_del);   }
	...
	if (!to_del.empty())
	{
		for (size_t i=0; i<to_del.size(); ++i){
			...
			fprintf(del_out_F, "%s\n", alns[0][to_del[i]].sequence().c_str());}
