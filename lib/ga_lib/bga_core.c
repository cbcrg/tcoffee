...
int do_generation(Population *L_POP,Parameter *L_PARAM, Statistic *STAT, int gen)
    ...
    process_fitness ( L_POP, 0, (L_PARAM->BGA)->MAXPOP, (L_PARAM->BGA)->GA_MAXIMISE);   
...
/******TERMINATION*********/
	...
	if (is_bi_parent(op, L_PARAM) )
	    ...
	    R= (a_better_than_b (s1,s2,(L_PARAM->BGA)->GA_MAXIMISE))?A1:A2;
	    s= (a_better_than_b (s1,s2,(L_PARAM->BGA)->GA_MAXIMISE))?s1:s2;
		...
		a=a_better_than_b ( s, ((L_POP+0)->CHROMOSOME[m_t[tot][0]])->score, (L_PARAM->BGA)->GA_MAXIMISE);
		b=a_better_than_b ( s, ((L_POP+0)->CHROMOSOME[m_t[tot][1]])->score, (L_PARAM->BGA)->GA_MAXIMISE);
	...
	if ( dup==0)
		...
		sp= (a_better_than_b (sp1,sp2,(L_PARAM->BGA)->GA_MAXIMISE))?sp1:sp2; 
		ref=((L_PARAM->BGA)->DYNAMIC_PARAM==0)?(int)STAT->score_best:sp; 
			
		m_t[tot][DELTA]=((a_better_than_b( s,ref,(L_PARAM->BGA)->GA_MAXIMISE)))?int_fabs ((int)(ref-s)):0;

		if ( current_best_score==current_worst_score==0)
		    current_best_score=current_worst_score=s;
		if ( a_better_than_b (s,current_best_score,(L_PARAM->BGA)->GA_MAXIMISE))
		    {current_best_score=s;
		    current_best_i=m_t[tot][Child];
		    }
		if ( a_better_than_b (current_worst_score,s,(L_PARAM->BGA)->GA_MAXIMISE))
