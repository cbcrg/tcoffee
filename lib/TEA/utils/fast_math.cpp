...
/*ADDED: THIS IS DEFINED IN UTIL.C IN line 8015, as well! FOR NOW DOUBLE DEFINITION SO WE ARE ABLE TO RUN TEA INDIVIDUALLY */ 
float LOOKUP_TEA (float x){
...
/*ADDED: THIS IS DEFINED IN UTIL.C IN line 8023, as well! FOR NOW DOUBLE DEFINITION SO WE ARE ABLE TO RUN TEA INDIVIDUALLY */
void LOG_PLUS_EQUALS_TEA (float *x, float y){

  if (x[0] < y)
    x[0] = (x[0] == LOG_ZERO || y - x[0] >= LOG_UNDERFLOW_THRESHOLD) ? y : LOOKUP_TEA(y-x[0]) + x[0];
  else
    x[0] = (y == LOG_ZERO || x[0] - y >= LOG_UNDERFLOW_THRESHOLD) ? x[0]  : LOOKUP_TEA(x[0]-y) + y;
}

/*ADDED: THIS IS DEFINED IN UTIL.C IN line 8031, as well! FOR NOW DOUBLE DEFINITION SO WE ARE ABLE TO RUN TEA INDIVIDUALLY */ 
float LOG_ADD_TEA (float x, float y){
  if (x < y) return (x == LOG_ZERO || y - x >= LOG_UNDERFLOW_THRESHOLD) ? y : LOOKUP_TEA((y-x)) + x;
  return (y == LOG_ZERO || x - y >= LOG_UNDERFLOW_THRESHOLD) ? x : LOOKUP_TEA((x-y)) + y;
}
