...
/*VARIABLES FOR THE MULTIPLE SEQUENCE ALIGNMENT*/
	...
	score=make_fasta_cdna_pair_wise ( A, ns, l_s, CL, diag);
...
/*FAST DP VARIABLES       */		
	int pos_j, last_i, last_j, ndiag, match1, match2; 
	int prev_j, new_j, j, len_j;
	int prev_i, new_i, i, len_i;
/*EVALUATION*/
	...
	for ( a=0; a< nstate; a++)
	  {
	    ...
	    for ( b=0; b< nstate; b++)
	      if( model[a][b]==-UNDEFINED)model[a][b]=UNDEFINED;
	  }
		...
		for (cur_state=START+1; cur_state< nstate; cur_state++)
		  {
			...
			if ( i<3 || pos_j<3)em=UNDEFINED;
		      ...
		      {
			if (trs1[i-3]=='x' || trs2[pos_j-3]=='x')em=UNDEFINED;
			else if (trs1[i-3]=='x' || trs2[pos_j-3]=='x')em=UNDEFINED;
			else em=emission[cur_state];
		      }
			  
		    
		    for (pc=best_pc=x=UNDEFINED, prev_state=START+1; prev_state< nstate; prev_state++)
		      {
			hsearch ( CMat,1, prev_state*mI+prev_i*mJ+prev_j,ADD);
			if(prev_i<0 || prev_j<0 ||prev_i>lenal[0] || prev_j>ndiag || len==UNDEFINED)
			 ...
			 n=hsearch ( CMat,0, prev_state*mI+prev_i*mJ+prev_j,FIND);
			 if (n==nstates-2)
			   {
	...
	for (pc=best_pc=x=UNDEFINED, state=START+1; state< nstate; state++)
	  {
	    ...
	    if (!is_defined_int(4,t,e,hsearch(Mat,0,state*mI+i*mJ+j,FIND),l))hsearch(Mat,0,state*mI+i*mJ+j,ENTER)=UNDEFINED;
