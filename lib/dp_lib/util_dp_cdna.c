...
#define cdna_gap(k)  ((k) <= 0 ? 0 : g+h*(k))    /* k-symbol indel cost */
...
int cdna_myers_miller_pair_wise (Alignment *A,int *ns, int **l_s,Constraint_list *CL )
	...
	(CL->get_dp_cost) (A, pos, ns[0], l_s[0],UNDEFINED, pos, ns[1], l_s[1],UNDEFINED,CL);
	...
	last=0;
	sapp=S;
        score=cdna_diff (A,ns, l_s, 0, l1, 0, l2, 0, 0, CL, pos);
	cdna_diff (NULL,ns, l_s, 0, l1, 0, l2, 0, 0, CL, pos); 
...
/*TG_MODE=2---> ---     ---*/
	 ...
	 if (N <= 0){if (M > 0) cdna_DEL(M);return cdna_gap(M);}
	 if (M <= 1)
	       { 
		  ...
		  return cdna_gap(N);
	       ...
	       if (tb > te) tb = te;
	       midc = (tb+h) + cdna_gap(N);
	       midj = 0;
    
	       for (j = 1; j <= N; j++)
	           { c = cdna_gap(j-1) + (CL->get_dp_cost) (A, pos, ns[0], l_s[0],s1, pos, ns[1], l_s[1],j-1+s2,CL)+ cdna_gap(N-j);
		   if (c > midc)
		      { midc = c;
		        midj = j;
		      }
	       ...
	       if (midj == 0)
	          {cdna_DEL(1) cdna_INS(N)}
	       else
	          {if (midj > 1) cdna_INS(midj-1);
		  REP;
		  if (midj < N) cdna_INS(N-midj);
		  }
...
/* Conquer: recursively around midpoint */
  ...
  if (type == 1)
    ...
    cdna_diff (A,ns, l_s, s1,midi, s2, midj, tb, CL->gop*SCORE_K, CL, pos); 
    cdna_diff (A,ns, l_s, s1+midi,M-midi, s2+midj, N-midj, CL->gop*SCORE_K,te, CL, pos); 
    }
  else
    { 
      cdna_diff (A,ns, l_s, s1,midi-1, s2, midj, tb,0, CL, pos); 
      cdna_DEL(2);
      cdna_diff (A,ns, l_s, s1+midi+1, M-midi-1,s2+midj, N-midj,0,te, CL, pos); 
    }
  return midc;
  }       
