...
static int IncompatibleMatState(MatState*S, MatState*E);
static void DisplayMatState ( MatState *S, char *s);
static void filter_matrix (char *s1,char *s2, double **M, double T);
static double*** probcons_pair_wise (char *seq1, char *seq2, int MODE);
static void display_state_matrix2 ( MatState ****M, Hmm *H, int l1, int l2, int state, char *string);
static void display_state_matrix ( double **M, Hmm *h, int l1, int l2, char *string);
...
Hmm* free_Hmm(Hmm*H)
  {
    ...
    for (a=0; a< H->nS; a++)
      {
	for ( b=0; b< H->nS; b++)vfree((H->S[a])->T[b]);
	vfree((H->S[a])->T);
	vfree(H->S[a]);
      }
    vfree (H->S);
...
/*                                                                               */
/*********************************************************************************/
int IncompatibleMatState(MatState*S, MatState*E)
{
  int di, dj;
  ...
  if ( S->st ==1 || E->st==1)return 0;
  else if ( di==0 && dj==0)return 0;
  else if (di==0 && dj>0)return 0;
  else if ( di>0 && dj==0) return 0;
  else if ( di==1 && dj==1)return 0;
...
void display_state_matrix2 ( MatState ****M,Hmm*H, int l1, int l2, int s, char *string)
{
  int a, b;

  fprintf (stderr, "\nMAtrix %s, %d %d", string, l1, l2);
  for ( a=0; a<=l1; a++)
    {
      fprintf ( stderr, "\n");
      for ( b=0; b<=l2; b++)
	{
	if ( M[a][b][s]->sc==H->forbiden) fprintf ( stderr, " %3c", '*');
	else fprintf ( stderr, " %3d", (int)M[a][b][s]->sc);
	}
    }
  fprintf (stderr, "\nMAtrix %s, %d %d (previous state)", string, l1, l2);
  for ( a=0; a<=l1; a++)
    {
      fprintf ( stderr, "\n");
      for ( b=0; b<=l2; b++)
	{
	if ( M[a][b][s]->sc==H->forbiden) fprintf ( stderr, " %3c", '*');
	else fprintf ( stderr, " %3d", (int)M[a][b][s]->pst);
	}
    }
}
void display_state_matrix ( double **M, Hmm *H, int l1, int l2, char *string)
{
  int a, b;
  ...
  for ( a=0; a<=l1; a++)
    {
      ...
      for ( b=0; b<=l2; b++)
	{
	if ( M[a][b]==H->forbiden) fprintf ( stderr, " %3c", '*');
	else fprintf ( stderr, " %3d", (int)M[a][b]);
	}
    }
}
